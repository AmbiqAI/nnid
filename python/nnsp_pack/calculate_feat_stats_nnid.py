...
def feat_stats_estimator(
    ...
    num_tot = len(fnames) * len(fnames[0]) * len(fnames[0][0]) / (num_group_ppls * num_sents)
    for batch, data in enumerate(dataset):
        tf.print(f"\rMean estimating (batch) {batch}/{num_tot}, ",
                     end = '')
        feats, masks = data
    ...
    for batch, data in enumerate(dataset):
        tf.print(
            f"\rSTD estimating (batch) {batch}/{num_tot}, ",
            end = '')
        feats, masks = data
        _, _, dim_feat = feats.shape
