...
DIM_INPUT = 40  # size of a single sample, e.g. an embedding of size 64
def model() -> tf.keras.Model:
    ...
    h_state = tf.keras.layers.Input(100, batch_size=1)
    c_state = tf.keras.layers.Input(100, batch_size=1)
    ...
    lstm_layer = tf.keras.layers.LSTM(
            ...
            stateful=False,
            unroll=True)
    dense_layer0 = tf.keras.layers.Dense(64, activation='linear')
...
def get_trained_model(args):
    ...
    neurons, _, layer_types, activations, num_context, num_dnsampl = out # pylint: disable=line-too-long
...
def main(args):
    ...
    h_state = np.random.randn(1,100)
    c_state = np.random.randn(1,100)
    ...
    def dataset_example(num_samples: int = 100):
        ...
        shape_data = (1, 6, 40,1) # inputs
        shape_state = (1, 100) # lstm has 16 neurons
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        '-a',
        '--nn_arch',
        default='nn_arch/def_id_nn_arch100_ti',
