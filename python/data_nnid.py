                ...
                for ntype in self.ntypes:
                    ...
                    for snr_db in self.snr_dbs_min_max:
                        ...
                        try:
                            ...
                            if len(speech_orig) > 160 * MAX_FRAMES:
                                speech = speech_orig[:160*MAX_FRAMES]
                                stime = 0
                                etime = len(speech)
                                ...
                                # print("short")
                                zz = np.zeros((160 * MAX_FRAMES-len(speech_orig),), dtype=np.float32)
                                speech = np.concatenate((zz, speech_orig))
                                stime = 160 * MAX_FRAMES-len(speech_orig)
                                etime = 160 * MAX_FRAMES
                                # print(speech)
                            target = 1
                            stimes += [stime]
                            etimes += [etime]
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        '-tr',
        '--train_dataset_path',
        default = 'data/nnid_train_id.yaml',
        help    = 'path to train data file')

    argparser.add_argument(
        '-tt',
        '--test_dataset_path',
        default = 'data/nnid_test_id.yaml',
